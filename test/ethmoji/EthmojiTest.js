...
contract('Ethmoji', accounts => { 
  ...
  beforeEach(async function() { 
    contractInstance = await contractDeclaration.new({ from: owner })
  ...
  describe('prices', () => { 
    it('for minting a composition with default prices', async function () { 
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) //mints token 1
      await contractInstance.mintTo(owner, compositionFee, imageHashTwo, { from: owner }) //mints token 2
    ...
    it('for minting a composition with set prices', async function () { 
      ...
      await contractInstance.mintTo(owner, compPrice1, imageHash, { from: owner }) //mints token 1
      await contractInstance.mintTo(owner, compPrice2, imageHashTwo, { from: owner }) //mints token 2
    ...
    it('after setting global composition fee', async function () {
      await contractInstance.setGlobalCompositionFee(new BigNumber(web3.toWei(1, "ether")), { from: owner })
      
      let newCompositionFee = await contractInstance.minCompositionFee()
      await contractInstance.mintTo(owner, newCompositionFee, imageHash, { from: owner }) //mints token 1
      await contractInstance.mintTo(owner, newCompositionFee, imageHashTwo, { from: owner }) //mints token 2

      let price = await contractInstance.getTotalCompositionPrice([1, 2])  
      
      let expectedMintingPrice = (((1 + 1) * developerFee ) / 100) * 100 //floating number math..
      price.toString().should.be.equal(web3.toWei(expectedMintingPrice, "ether"))
    })
  })

  describe('avatar', () => { 
    it('sets default avatar on minting first/new token', async function () { 
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) // token id 1
      let tokenOwner = await contractInstance.ownerOf(1)

      tokenOwner.should.be.equal(owner)

      let avatar = await contractInstance.getAvatar(owner)
      avatar.should.bignumber.be.equal(1)
    })

    it('sets default avatar only when there was no previous avatar', async function () { 
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) // token id 1
      await contractInstance.mintTo(owner, compositionFee, imageHashTwo, { from: owner }) // token id 2

      let avatar = await contractInstance.getAvatar(owner)
      avatar.should.bignumber.be.equal(1)
    })

    it('can be set with new ethmoji ', async function () { 
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) // token id 1
      await contractInstance.mintTo(owner, compositionFee, imageHashTwo, { from: owner }) // token id 2

      await contractInstance.setAvatar(new BigNumber(2), { from: owner })

      let avatar = await contractInstance.getAvatar(owner)
      avatar.should.bignumber.be.equal(2)
    })

    it('removed when avatar ethmoji is transferred', async function () { 
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) // token id 1

      let tokenOwner = await contractInstance.ownerOf(new BigNumber(1), { from: owner })
      tokenOwner.should.be.equal(owner)

      let avatar = await contractInstance.getAvatar(owner)
      avatar.should.bignumber.be.equal(1)

      await contractInstance.transfer(purchaser, new BigNumber(1), { from: owner })

      tokenOwner = await contractInstance.ownerOf(new BigNumber(1), { from: owner })
      tokenOwner.should.be.equal(purchaser)

      avatar = await contractInstance.getAvatar(owner)
      avatar.should.bignumber.be.equal(0)
    })
  })
  ...
  describe('minting a base token', () => { 
    ...
    it('emits correct log', async function () { 
      tx = await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner })
  ...
  describe('uniqueness', () => {
    ...
    beforeEach(async function () { 
      for(let i = 1; i <= 10; i ++ ) { 
        await contractInstance.mintTo(owner, compositionFee, i, { from: owner })
      }
    })
    ...
    it('checks that uniqueness fails on compositions with more than 100 layers', async function () { 
      for(let i = 1; i <= 101; i ++ ) { 
        let imageHash = i+10;
        await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner })
      }
      let compLayers = Array.apply(null, Array(101)).map(function (_, i) { return i+1; });
      await expectThrow(contractInstance.isValidComposition(compLayers, imageHashTwo))
    })
  ...
  describe('profits', () => { 
    it('gives owner of emoji profits from compostion emojis', async function () {
      ...
      await contractInstance.mintTo(purchaser, compositionFee, imageHash, { from: owner }) //mints token 1
      await contractInstance.mintTo(owner, compositionFee, imageHashTwo, { from: owner }) //mints token 2
    ...
    it('gives owner of emoji profits from compostion emojis with set composition price', async function () { 
      ...
      await contractInstance.mintTo(owner, compositionFee, imageHash, { from: owner }) //mints token 1
      await contractInstance.mintTo(purchaser, compositionFee, imageHashTwo, { from: owner }) //mints token 2
